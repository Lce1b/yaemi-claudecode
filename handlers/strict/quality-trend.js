...
const TREND_FILE = path.join(config.DATA_DIR, 'quality-trends.json'), MAX_HISTORY = 20;
...
module.exports = {
  ...
  async run(event, ctx) {
    ...
    const qs = Math.min(100, Math.round((editedCount * 10) + (Math.log2(Math.max(1, tIn + tOut)) * 5)));
    ...
    let sum = 0; for (let i = 0; i < trends.sessions.length; i++) sum += trends.sessions[i].files_edited;
    ...
    const lt = trends.sessions.slice(-3); if (lt.length >= 3) { const sc = lt.map(function(s){return s.quality_score||0;}); trends.summary.trendWarning = sc[0] > sc[1] && sc[1] > sc[2] ? 'quality_declining' : null; }
    saveTrends(trends);
